...
public class DrawingPlacer : MonoBehaviourPun
{
    ...
    private void HandleModeSwitchInput()
    {
        ...
        if (m == ModeManager.AppMode.TabPlace)      SwitchMode(PlayerMode.TabPlacement);
        else if (m == ModeManager.AppMode.Erase)    SwitchMode(PlayerMode.Eraser);
        else                                        SwitchMode(PlayerMode.Default3DDraw);
    }
