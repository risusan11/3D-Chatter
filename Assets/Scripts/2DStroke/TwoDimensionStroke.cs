...
public class TwoDimensionStroke : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // ────────────────────────────────────────────
        // ✅ 【修正1】色の復元：PlayerPrefs → colorPicker → lineColor の順で同期
        //
        // 旧コード：lineColor（Inspector値）と colorPicker（プレハブ初期色=赤）が
        //           Start時点で食い違い、描くたびに意図しない色が使われていた。
        //
        // 新コード：
        //   1. PlayerPrefsに保存済みの色があれば復元
        //   2. なければ colorPicker の現在色を採用（プレハブ初期色を正とする）
        //   3. lineColor と colorPicker を必ず同じ値に揃えてから開始
        // ────────────────────────────────────────────
        ...
        {
            // 初回起動：colorPicker の初期色を正として採用（プレハブ設定に従う）
            lineColor = colorPicker.color;
        }

        // colorPicker の表示色を lineColor に合わせて強制上書き
        if (colorPicker != null)
        {
    ...
    void _addLineObject()
    {
        ...
        // ✅ 【新規】最初のストロークのときにキャンバスのアスペクト比を保存
        //    これがないと StrokeTextureRenderer が縦方向のブラシ太さを正しく計算できない。
    ...
    /// <summary>
    /// drawingCamera が現在画面に映しているワールド空間の横幅を返す。
    /// </summary>
