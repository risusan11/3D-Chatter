...
public class ObjectPlacer : MonoBehaviourPun
{
    ...
    {
        // 数字キー 1〜9：番号選択（司令塔には SetMode で必ず ObjPlace、同じ番号でトグル解除）
        for (int i = 0; i < 9 && i < catalog.Count; i++)
        {
            if (Input.GetKeyDown(KeyCode.Alpha1 + i))
            {
                if (selectedIndex == i && ModeManager.Instance.Current == ModeManager.AppMode.ObjPlace)
                {
                    // 同じ番号をもう一回 → 解除
                    ModeManager.Instance.RequestMode(ModeManager.AppMode.ObjPlace);
                }
                else
                {
                    // 違う番号 or 別モードから → その番号で配置モードに
                    selectedIndex = i;
                    manualRotationY = 0f;
                    ModeManager.Instance.SetMode(ModeManager.AppMode.ObjPlace);
                    BuildPreview();   // ← 追加：番号が変わったらプレビューを作り直す
                }
                return;
            }
        }

        // 司令塔のモードを、自分の内部モードに翻訳
        var m = ModeManager.Instance.Current;
        Mode target = Mode.None;
        if (m == ModeManager.AppMode.ObjPlace)       target = Mode.Place;
        else if (m == ModeManager.AppMode.ObjDelete) target = Mode.Delete;
        else if (m == ModeManager.AppMode.ObjMove)   target = Mode.Move;
