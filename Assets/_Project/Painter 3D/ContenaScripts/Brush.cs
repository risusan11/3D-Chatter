...

{
...

    {
        ...
        public Stroke BeginStroke(Transform brushTip)
        {
            ...
            m_ActiveStroke = Stroke.GetNewStroke(
                Instantiate(Painter3DResourceManager.Instance.ActiveStrokeRenderer),
                m_PaintManager.m_PaintingLayer, m_BrushSize, m_Colour);

            // ★ 順序変更：AddStroke より先に SetParent(null) してワールド座標系に固定
            m_ActiveStroke.transform.SetParent(null);
            m_ActiveStroke.transform.position = Vector3.zero; // ワールド原点に配置

            // Canvasには記録だけさせる（親子関係はすでにnull）
            m_PaintManager.ActiveCanvas.m_Strokes.Add(m_ActiveStroke);
            m_ActiveStroke.name = "Stroke " + m_PaintManager.ActiveCanvas.m_Strokes.Count;
